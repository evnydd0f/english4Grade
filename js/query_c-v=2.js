...
result.requestUrl = "http://47.94.127.138";
result.tjUrl = "http://47.94.127.120";
...
window.onload = function() {
	...
	util.get("submitButton").onclick=function(){
		...
		_hmt.push(['_trackEvent', 'queryc', 'click', c+'-q', 1]);
    ...
    result.callback = function(data){
		...
		if(data){
			if(data.s||data.s==0){
				...
				_hmt.push(['_trackEvent', 'querycSuccess', 'result', c+'-qs', 1]);
